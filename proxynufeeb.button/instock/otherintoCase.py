...
class otherintoTest(unittest.TestCase):
    ...
    def testotheritemSeriesinto(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%……&*（）12；other into series item summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"other into series item comment中文测试饕餮壹贰！@#￥%……&*（）12；")
    ...
    def testotherInto(self):
        ...
        try:
            #摘要
            summid=pageid+browser.xmlRead(dom,'edSummary',0)
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%……&*（）12；other into summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"other into comment中文测试饕餮壹贰！@#￥%……&*（）12；")
