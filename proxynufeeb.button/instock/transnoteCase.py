...
class transnoteTest(unittest.TestCase):
    ...
    def testtranitemseries(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；trans item series summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"trans item series comment中文测试饕餮壹贰！@#￥%^ &*（）12；")
    ...
    def testtransNote(self):
        ...
        try:
            ...
            #摘要
            summid=pageid+browser.xmlRead(dom,'edSummary',0)
            browser.findId(self.driver,summid).send_keys("trans note summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys("trans note comment")
