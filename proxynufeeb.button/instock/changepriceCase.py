...
class changepriceTest(unittest.TestCase):
    ...
    def testchangePrice(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"change price note summary中文测试饕餮壹贰！@#￥%……&*（）12；")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"中文测试饕餮壹贰！@#￥%……&*（）12；change price note comment")
