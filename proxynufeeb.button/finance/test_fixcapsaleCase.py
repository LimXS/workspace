...
class fixcapsaleTest(unittest.TestCase):
    u'''财务-固定资产-固定资产变卖'''
    ...
    def test_fixcapSale(self):
        ...
        moduledd=browser.xmlRead(dom,'moduledd',8)
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance fixcap sale summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance fixcap sale commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
            ...
            browser.getpicture(self.driver,filename+u"财务-固定资产-固定资产变卖.png")
