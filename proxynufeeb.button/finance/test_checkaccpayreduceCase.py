...
class checkaccpayreduceTest(unittest.TestCase):
    ...
    def test_checkaccpayReduce(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance check account pay reduce summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance account pay reduce commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
