...
class checkfundaddTest(unittest.TestCase):
    u'''财务-调账-资金增加'''
    ...
    def test_checkfundAdd(self):
        ...
        #页面id
        pageurl=browser.xmlRead(dom,"checkfundaddurl",0)
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance check fund add summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance check fund add commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
            ...
            browser.getpicture(self.driver,filename+u"财务-调账-资金增加.png")
