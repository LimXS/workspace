...
class checkaccrereduceTest(unittest.TestCase):
    u'''财务-调账-应收款减少'''
    ...
    def test_checkaccreReduce(self):
        u'''财务-调账-应收款减少'''
        ...
        #页面id
        pageurl=browser.xmlRead(dom,"checkaccrereduceurl",0)
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance check account reduce add summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance account reduce add commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
            ...
            browser.getpicture(self.driver,filename+u"财务-调账-应收款减少.png")
