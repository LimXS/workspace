...
class fixcapbuyTest(unittest.TestCase):
    ...
    def test_fixcapBuy(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance fixcap buy summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance fixcap buy commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
