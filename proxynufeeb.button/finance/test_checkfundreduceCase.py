...
class checkfundreduceTest(unittest.TestCase):
    ...
    def test_checkfundReduce(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance check fund reduce summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance check fund reduce commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
