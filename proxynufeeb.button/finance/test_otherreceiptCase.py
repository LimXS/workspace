...
class otherreceiptTest(unittest.TestCase):
    u'''财务-其他收入单'''
    ...
    def test_otherReceipt(self):
        ...
        #页面id
        pageurl=browser.xmlRead(dom,"receurl",0)
        ...
        try:
            ...
            #金额
            js="$(\"input[id$=grid_total]\").val('2000')"
            browser.delaytime(1)
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰！@#￥%^ &*（）12；finance other receipt summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance other receipt commentid中文测试饕餮壹贰！@#￥%^ &*（）12；")
            ...
            browser.getpicture(self.driver,filename+u"财务-其他收入单.png")
