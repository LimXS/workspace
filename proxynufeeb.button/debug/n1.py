...
def getsubdata():
    ...
    #审核
    fc=open(r"E:\vccheck.txt")
    item=fc.read().strip()
    all2=re.findall("values\('(.*?)'",item)

    fc2=open(r"E:\vccheckitem.txt")
    item=fc2.read().strip()
...
def sendpro(num):
    ...
    fc=open(r"D:\cookies\eshop8.txt")
    nskcookie=fc.read()
    browser.delaytime(1)
    ...
    nskcookie3=f3.read()
    browser.delaytime(1)
    f4=open(r"D:\cookies\eshop6.txt")
    nskcookie4=f4.read()
    f5=open(r"D:\cookies\eshop5.txt")
    nskcookie5=f5.read()
    global headers
    global headers2
    global headers3
