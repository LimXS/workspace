...
class saleoutTest(unittest.TestCase):
    ...
    def testsaleitemseriseOut(self):
        ...
        try:
            ...
            f=open(r'C:\workspace\proxynufeeb.button\stock\seriesNoitem','r')
            checkNo=f.read()
            ...
            browser.findId(self.driver,edSummary).send_keys(u"中文测试饕餮壹贰123！@#￥%……&*（）()； abcButton saleout series item summary")

            #附加说明
            edComment=pageid+browser.xmlRead(dom,"edComment",0)
            browser.findId(self.driver,edComment).send_keys(u"中文测试饕餮壹贰123！@#￥%……&*（）()； abcButton saleout series item comment")
    ...
    def testsaleOut(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys("sale out summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys("sale out commentid")
            ...
            filename=browser.xmlRead(dom,'filename',0)
            browser.delaytime(1)
            browser.getpicture(self.driver,filename+u"批零-销售出库单-普通商品.png")
