...
class entrustaccTest(unittest.TestCase):
    ...
    def testentrustAcc(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文测试饕餮壹贰123！@#￥%……&*（）()； abcentrust acc summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"中文测试饕餮壹贰123！@#￥%……&*（）()； abcentrust acc commentid")
