...
class fixcapdeprTest(unittest.TestCase):
    ...
    def test_fixcapDepr(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.finance fixcap dis summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance fixcap dis commentid中文蘩軆饕餮！@#￥%……&*（）？； 。.")
