...
class menexitranTest(unittest.TestCase):
    ...
    def test_menexiTran(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.finance men exit tran summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance men exit tran commentid中文蘩軆饕餮！@#￥%……&*（）？； 。.")
