...
class checkfundaddTest(unittest.TestCase):
    ...
    def test_checkfundAdd(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.finance check fund add summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"finance check fund add commentid中文蘩軆饕餮！@#￥%……&*（）？； 。.")
