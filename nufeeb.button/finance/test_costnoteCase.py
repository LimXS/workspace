...
class costnoteTest(unittest.TestCase):
    ...
    def test_costNote(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"finance cost note summary中文蘩軆饕餮！@#￥%……&*（）？； 。.")

            #附加说明
            commentid=pageid+browser.xmlRead(dom2,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.finance cost note commentid")
