...
class saleexchangeTest(unittest.TestCase):
    ...
    def testsaleExchange(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.sale exchange  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"sale exchange comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
