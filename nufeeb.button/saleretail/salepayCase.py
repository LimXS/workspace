...
class salepayTest(unittest.TestCase):
    ...
    def testsalePay(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.sale paynote  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"sale paynote commentid中文蘩軆饕餮！@#￥%……&*（）？； 。.")
