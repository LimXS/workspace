...
class innerdrawTest(unittest.TestCase):
    ...
    def test_innerDraw(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.inner draw note  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"inner draw note comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
