...
class getmorenoteTest(unittest.TestCase):
    ...
    def testgetitemseriesMore(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.more note item series summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"more note item series comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
    ...
    def testgetmoreNote(self):
        ...
        try:
            ...
            #摘要
            summid=pageid+browser.xmlRead(dom,'edSummary',0)
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.less note  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"less note comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
