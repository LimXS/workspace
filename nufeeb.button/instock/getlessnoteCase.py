...
class getlessnoteTest(unittest.TestCase):
    ...
    def testgetitemseriesLess(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.less note item series summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"less note item series comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
    ...
    def testgetlessNote(self):
        ...
        try:
            ...
            #摘要
            summid=pageid+browser.xmlRead(dom,'edSummary',0)
            browser.findId(self.driver,summid).send_keys(u"less note  summary中文蘩軆饕餮！@#￥%……&*（）？； 。.")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.less note comment")
