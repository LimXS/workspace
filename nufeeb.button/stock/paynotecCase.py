...
class stockpayTest(unittest.TestCase):
    ...
    def teststockPay(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.stock paynote  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"stock paynote commentid中文蘩軆饕餮！@#￥%……&*（）？； 。.")
