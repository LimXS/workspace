...
class stockintocateTest(unittest.TestCase):
    ...
    def testitemseriesCate(self):
        ...
        try:
            ...
            f=open(r'C:\workspace\nufeeb.button\stock\seriesNoitem','w')
            f.write(itemser)
            f.close()
            ...
            browser.findId(self.driver,edSummary).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.Button stockinto series item summary")

            #附加说明
            edComment=pageid+browser.xmlRead(dom,"edComment",0)
            browser.findId(self.driver,edComment).send_keys(u"Button stockinto series item comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
    ...
    def teststockintoCate(self):
        ...
        try:
            ...
            #摘要
            edSummary=pageid+browser.xmlRead(dom,"edSummary",0)
            browser.findId(self.driver,edSummary).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.Button stockinto summary")

            #附加说明
            edComment=pageid+browser.xmlRead(dom,"edComment",0)
            browser.findId(self.driver,edComment).send_keys(u"Button stockinto comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
...
'''
    def assnskDraft(self):
        ...
        try:
            ...
            browser.delaytime(1)
            browser.getpicture(self.driver,filename+u"进货-新增进货订单-断言进货订单保存为草稿.png")
