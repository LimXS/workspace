...
class stockexchangeTest(unittest.TestCase):
    ...
    def teststockExchange(self):
        ...
        try:
            ...
            browser.findId(self.driver,summid).send_keys(u"中文蘩軆饕餮！@#￥%……&*（）？； 。.stock exchange  summary")

            #附加说明
            commentid=pageid+browser.xmlRead(dom,'edComment',0)
            browser.findId(self.driver,commentid).send_keys(u"stock exchange comment中文蘩軆饕餮！@#￥%……&*（）？； 。.")
