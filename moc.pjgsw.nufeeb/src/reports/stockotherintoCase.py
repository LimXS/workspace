...
class stockotherintoTest(unittest.TestCase):
    u'''报表-库存报表-其他入库单统计'''
    ...
    def teststockotherInto(self):
        ...
        for j in range(len(pagedic["itemList"])):
            ...
            if number[:3]=='QR-':
                ...
                noteurl="http://beefun.wsgjp.com/Beefun/Bill/OtherStockBill.gspx?Vchtype="+str(Vchtype)+"&Vchcode="+str(Vchcode)+"&Mode=Read"
        ...
        self.assernote(notesummoney,notetotalmoney,u"单据中心其他入库单总金额不相同（单据中心和本页面统计）","note total other into money  ok","单据中心和本页面统计总金额")
        '''
        ...
        #报表-库存报表-其他入库单统计
        print u"报表-库存报表-其他入库单统计...................................."
        urlid="http://beefun.wsgjp.com/Beefun/Report/OtherOrderQuery.gspx?mode=3"
        ...
        for temp in totaldata["itemList"]["rows"]:
            m=m+1
            print u"其他入库单统计第"+str(m)+"行:"+str(temp[2])
            #取每种商品入库明细单据
            overday=stamp[1]*1000
            today=stamp[0]*1000
            #页面id
            detailidurl="http://beefun.wsgjp.com/Beefun/Report/OtherOrderQueryDetails.gspx?mode=3"
            ...
            for p in detaillist:
                n=n+1
                data=re.findall("(.*?),",p+",")
                print u"其他入库单明细第"+str(n)+"行:"+str(data[4])
                try:
                ...
                self.assernote(redenum,ornums,u"其他入库数量不相同（明细数据和原始单据）","reportdetail other into number ok",data[4])
        ...
        self.assernote(totnum,reporttotalnum,u"数量不相同（报表页面和它页面的统计）","reportself number ok",u"其它入库单统计")
        ...
        self.assernote(totmoney,reporttotalmoney,u"金额不相同（报表页面和它页面的统计）","reportself money ok",u"其它入库单统计")
        ...
        print u"单据中心和其他入库单统计数据...................."
        #单据中心和其他入库单统计数据
        if len(notelisttotal)!=0:
            print u"单据中心统计到其他入库单统计报表有多余或者遗漏"
            print u"单据中心数据总条数："+str(notetotalnum)
            print u"其他入库单统计数据总条数："+str(int(detailtotal))
        else:
