...
def movie_list(url,headers):
    try:
            ...
            handle_list=[]
            #获取图片进行保存
            pic=re.findall("src=\"(.*?)\" alt",url_list.text)
            n=0
            lock.acquire()
            for m in pic:
                print m
                ...
                add=re.findall("c/(.*?)$",m)
                add2='.'.join(add)
                filename=name+"\pic"+str(add2)


                resp=requests.get(m,verify=False)
                c=resp.content
                f=open(filename,'wb')
                f.write(c)
                f.close()
            ...
            for scores in scores_list:
                if scores!='':
                    ...
                    if scores>=8.5:
                        ...
                        tit=title_list[flag]
                        comm=commit_list[flag].get_text().strip(' ')
                        sco=str(scores)
                        handle_list.append(tit)
                        handle_list.append(sco)
                        handle_list.append(comm)
                        handle_list.append(sum)
